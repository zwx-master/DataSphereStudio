  ...
  <div class="console-page">
    ...
    <div class="console-page-content">
      <div class="console-page-content-head">
        <div>
          <span class="console-page-content-title">{{ $t('message.console.title') }}</span>
          <span class="console-page-content-info">{{ $t('message.console.info', {num: '13'}) }}</span>
        </div>
      </div>
...
export default {
  data() {
    return {
      sideNavList: [{
        ...
        icon: 'ios-options',
        children: [
          { key: '1-1', name: this.$t('message.console.sideNavList.function.children.globalHistory'), path: '/console/globalHistory' },
          { key: '1-2', name: this.$t('message.console.sideNavList.function.children.resource'), path: '/console/resource' },
          { key: '1-3', name: this.$t('message.console.sideNavList.function.children.setting'), path: '/console/setting' },
          { key: '1-4', name: this.$t('message.console.sideNavList.function.children.dateReport'), path: '/console/globalValiable' },
          { key: '1-5', name: this.$t('message.console.sideNavList.function.children.globalValiable'), path: '/console/FAQ' },
        ],
      }],
      breadcrumbFirstName: this.$t('message.console.sideNavList.function.name'),
      breadcrumbSecondName: this.$t('message.console.sideNavList.function.children.globalValiable'),
      contentHeight: 0,
    };
  },
  created() {
  ...
  beforeRouteEnter(to, from, next) {
    if (to.name === 'FAQ' && from.name === 'Home') {
      next((vm) => {
        vm.breadcrumbFirstName = this.$t('message.console.sideNavList.function.name');
        vm.breadcrumbSecondName = this.$t('message.console.sideNavList.function.children.globalValiable');
      });
