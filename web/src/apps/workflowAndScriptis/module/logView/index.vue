<template>
  <div class="log-view">
    <div
      ...
      <span class="log-character">{{ $t('message.logView.taskId') }}</span>
      <Input
        v-model="jobId"
      ...
      <Button
        ...
        icon="search"
        :loading="isLoading"
        @click="search()">{{ $t('message.logView.search') }}</Button>
    </div>
    <div class="log-editor-wrap">
      <div class="log-tools">
        <div class="log-tools-control">
          ...
          <Input
            v-model="searchText"
            :placeholder="$t('message.logView.filter')"
            suffix="ios-search"
            size="small"
    ...
    async search() {
      if (!this.isLoading) {
        ...
        if (!this.jobId || !JOBID_REGEXP.test(this.jobId)) {
          this.$Notify.error({
            title: this.$t('message.logView.error'),
            message: this.$t('message.logView.inputTaskId'),
            duration: 1000,
          });
