<template>
  <div class="juri-serchbar-box">
    <h3 style="width:80%;display: inline-block;" >{{$t('message.userTable.permissionsManagement')}}</h3>
    <Button
      type="success"
      @click="creater"
    >{{$t('message.userTable.create')}}</Button>
    <!-- <formserch
      @click-serach="search"
...
export default {
  ...
  data() {
    return {
      ...
      menudata: {
        columns: [],
        datalist: [],
        title: this.$t('message.userTable.menuPermissions'),
      },
      assembly: {
        columns: [],
        datalist: [],
        title: this.$t('message.userTable.componentAccessPermissions'),
      },
      searchBar: {
        title: this.$t('message.userTable.permissionsName'),
        searchText: "",
        engine: 0,
        status: 0
      },
  ...
  methods: {
    init() {
      api.fetch('/dss/getWorkspaceMenuPrivs', {
    ...
    tabletittle(roles){
      ...
      arr.unshift({ title: this.$t('message.userTable.permissionsName'), key: "name", align: "center"})
      arr.push({title: this.$t('message.userTable.action'), slot: "action", align: "center"})
      return arr
    },
    search() {
    },
