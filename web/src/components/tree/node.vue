...
<template>
  ...
  >
    <div
      ...
      <span
        v-if="!node.isLeaf"
        class="we-tree-node__expand-icon"/>
      <span
        :class="supportIcon(node)"
        class="we-tree-node__icon"/>
...
import viewModal from '@/components/functionView';
import SUPPORTED_LANG_MODES from '@/apps/workflowAndScriptis/config/scriptis.js';
export default {
  name: 'WeTreeNode',
  components: {
    viewModal,
